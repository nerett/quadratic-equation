...
// structs
    ...
    if( conduct_unit_test( 8,     8,       -4,       -4,     TWO,              1,               -0.5 ) == false ) N_errors++;  //!TODO подстановка корней
...
bool conduct_unit_test( int test_number, double coeff_a, double coeff_b, double coeff_c, number_of_roots expected_N_roots, double expected_root_1, double expected_root_2 )
{
    ...
    QuadraticEquation tested_equation = {};
    ...
    if( ( tested_equation.roots != expected_N_roots )
        || ( tested_equation.root_1 != expected_root_1 )
        || ( tested_equation.root_2 != expected_root_2 ) )
    {
        if( tested_equation.roots == expected_N_roots )
        {
        ...
        display_error_message( test_number, &tested_equation, expected_N_roots, expected_root_1, expected_root_2 );
...
static void display_error_message( int test_number, struct QuadraticEquation* tested_equation, number_of_roots expected_N_roots, double expected_root_1, double expected_root_2 )
{
    printf("\n[ERROR] in test №%d! \n", test_number);
